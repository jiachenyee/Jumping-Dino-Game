...
//  Vision Thing
//
//  Created by Jia Chen Yee on 4/1/23.
//
...
class ViewController: NSViewController, AVCaptureVideoDataOutputSampleBufferDelegate {
    ...
    var latestPoint: PointPair?
    
    @IBOutlet weak var jumpLabel: NSBox!
...
//    var detectedFaceLandmarksShapeLayer: CAShapeLayer?
    ...
    var referencePoint: PointPair?
    
    @IBAction func callibrateButtonPressed(_ sender: Any) {
        if let latestPoint = latestPoint {
            referencePoint = latestPoint
            
            let refY = latestPoint.point.y - 25
            let path = CGMutablePath()
            
            path.move(to: CGPoint(x: 0, y: refY))
            path.addLine(to: CGPoint(x: 3000, y: refY))
            
            faceOverShapeLayer?.path = path
        }
...
//        self.present(alertController, animated: true)
    ...
    fileprivate func setupVisionDrawingLayers() {
        ...
        let faceOverShapeLayer = CAShapeLayer()
        faceOverShapeLayer.name = "Face Over Line Shape Layer"
        faceOverShapeLayer.bounds = captureDeviceBounds
...
//        }
    ...
    fileprivate func addIndicators(to faceRectanglePath: CGMutablePath, faceLandmarksPath: CGMutablePath, for faceObservation: VNFaceObservation) {
        ...
        faceRectanglePath.move(to: CGPoint(x: faceBounds.midX, y: faceBounds.maxY))
        faceRectanglePath.addLine(to: CGPoint(x: faceBounds.midX, y: faceBounds.minY))
        
        faceRectanglePath.move(to: CGPoint(x: faceBounds.minX, y: faceBounds.midY))
        faceRectanglePath.addLine(to: CGPoint(x: faceBounds.maxX, y: faceBounds.midY))
...
//        faceLandmarksShapeLayer.path = faceLandmarksPath
    ...
    let cgEventSource = CGEventSource(stateID: CGEventSourceStateID.combinedSessionState)
    
    func space(){
        let keyDown = CGEvent(keyboardEventSource: cgEventSource, virtualKey: CGKeyCode(49), keyDown: true)
        
        keyDown?.post(tap: CGEventTapLocation.cgSessionEventTap)
        
        let keyUp = CGEvent(keyboardEventSource: cgEventSource, virtualKey: CGKeyCode(49), keyDown: false)
